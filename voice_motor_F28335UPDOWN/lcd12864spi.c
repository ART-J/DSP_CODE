////////////////////////////////////////////////////////////////////////////////////////////////////
//                                                   使用lcd12864.c之前先把以下函数声明放到main.c程序中
// 调用lcd12864.c函数需要用到的声明
...
#define SCLK_H    GpioDataRegs.GPBSET.bit.GPIO57 = 1;  	 //时钟线置高
#define SCLK_L    GpioDataRegs.GPBCLEAR.bit.GPIO57 = 1;  //时钟线置低
...
//* 重要说明：此函数把 ---GPIO56---引脚作为串行数据输出，如需变动，需修改GPIO引脚寄存器
//********************************************************************************
void SID(unsigned char data)
{
	EALLOW;
	GpioDataRegs.GPBDAT.bit.GPIO56 = ((data&0x80)>>7);
	//GpioDataRegs.GPxDAT.bit.GPIOxx = ((data&0x80)>>7);//示例
	EDIS;
}
